...
def simulate_and_plot(ax, force_lacI, force_tetR, force_cI, title):
    ...
    # solve
    solution = odeint(repressilator_odes_forced, y0, t, args=(force_lacI, force_tetR, force_cI))
    ...
    if steady_start_time < t[-1]:
        ax.axvspan(0, steady_start_time, alpha=0.12, color='orange', label='Transient', zorder=0)
        ax.axvspan(steady_start_time, t[-1], alpha=0.12, color='lightblue', label='Steady state', zorder=0)
    else:
        ax.axvspan(0, t[-1], alpha=0.12, color='orange', label='Transient (Not Settled)', zorder=0)
    
    ax.plot(t, p_tetR, color='#FF8C00', linewidth=1.8, 
            label='TetR' + (' (forced)' if force_tetR else ''), alpha=0.9, zorder=3)
    ax.plot(t, p_lacI, color='#DC143C', linewidth=1.8, 
            label='LacI' + (' (forced)' if force_lacI else ''), alpha=0.9, zorder=3)
    ax.plot(t, p_cI, color='#0066CC', linewidth=1.8, 
            label='λcI' + (' (forced)' if force_cI else ''), alpha=0.9, zorder=3)
...
simulate_and_plot(axes[0], force_lacI=True, force_tetR=False, force_cI=False, 
                  title='(a) LacI forced')

simulate_and_plot(axes[1], force_lacI=True, force_tetR=True, force_cI=False, 
                  title='(b) LacI and TetR forced')

simulate_and_plot(axes[2], force_lacI=True, force_tetR=True, force_cI=True, 
