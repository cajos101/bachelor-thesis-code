...
if mean_val is not None:
    ax.axhline(y=mean_val, color='gray', linestyle=':', linewidth=1, alpha=0.6, zorder=2)
    ax.text(window_start + 0.2, mean_val - 0.2, 'Oscillation Center', color='gray', fontsize=11)
...
if len(peaks_window) >= 2:
    ...
    ax.annotate('', xy=(amp_x, peak_val), xytext=(amp_x, mean_val),
                arrowprops=dict(arrowstyle='<->', color=c_annot, lw=1.5))
    
    ax.text(amp_x + 0.3, (peak_val + mean_val)/2, 
            f'Amplitude\n$A = {amplitude:.2f}$', 
            fontsize=12, color=c_annot, va='center')

    y_dims = peak_val + 0.4
    

    ax.plot([t_start_period, t_start_period], [peak_val, y_dims], color='gray', linestyle=':', lw=1, alpha=0.8)
    ax.plot([t_end_period, t_end_period], [peak_val, y_dims], color='gray', linestyle=':', lw=1, alpha=0.8)
    
    ax.annotate('', xy=(t_start_period, y_dims), xytext=(t_end_period, y_dims),
                arrowprops=dict(arrowstyle='<->', color=c_annot, lw=1.5))

    ax.text((t_start_period + t_end_period)/2, y_dims + 0.05, 
            f'Period $T = {period_T:.2f}$', 
            ha='center', va='bottom', fontsize=12, color=c_annot,)

    # peaks
    ax.plot(t_window[peaks_window], p_lacI_window[peaks_window], 'o', 
            color='white', markeredgecolor=c_lacI, markeredgewidth=1.5, markersize=7, zorder=4)

    ax.plot([], [], 'o', markeredgecolor=c_lacI, markerfacecolor='white', 
            markersize=7, markeredgewidth=1.5, label='Peaks')
...
ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', frameon=True, fancybox=True, 
