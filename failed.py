...
if len(peaks_lacI) >= 4:
    def draw_period_dim(idx_start, idx_end, y_level, label):
        ...
        ax.plot([t_s, t_s], [y_s, y_level], color='gray', linestyle=':', lw=1, alpha=0.8)
        ax.plot([t_e, t_e], [y_e, y_level], color='gray', linestyle=':', lw=1, alpha=0.8)
        
        ax.annotate('', xy=(t_s, y_level), xytext=(t_e, y_level),
                    arrowprops=dict(arrowstyle='<->', color=c_annot, lw=1.5))
        
        period = t_e - t_s
        txt = ax.text((t_s + t_e)/2, y_level + 0.05, f'{label} = {period:.2f}', 
    ...
    for i, idx in enumerate(peaks_lacI[:4]):
        ...
        ax.plot(t_p, y_p, 'o', color='white', markeredgecolor = c_lacI, 
                markeredgewidth=1.5, markersize = 7, zorder = 4)
        ax.text(t_p, y_p + 0.2, str(i+1), ha='center', va='bottom', 
