...
if __name__ == '__main__':
    ...
    # plot
    contour = ax.contourf(T_values, A_values, synchronization_strength, levels=50, cmap='viridis', zorder=0)
    
    # entrainment lines
    ax.axvline(x=T_natural, color='white', linestyle='--', linewidth=1, alpha=0.8, label=f'Natural Period $T_0$', zorder=2)
    ax.axvline(x=T_natural/2, color='white', linestyle=':', linewidth=1, alpha=0.8, label=f'1:2 Resonance', zorder=2)
    ax.axvline(x=T_natural*2, color='white', linestyle=':', linewidth=1, alpha=0.8, label=f'2:1 Resonance', zorder=2)
    ...
    # legend
    ax.legend(bbox_to_anchor=(1.0, 1.0), loc='upper right', frameon=True, fancybox=True, 
              fontsize=12, framealpha=0.2, edgecolor='white', labelcolor='white')
