...
ax.plot(t, alpha_values, color='#DC143C', linewidth=2, label=r'$\alpha(t)_{\text{trapez}}$')

ax.axhline(y=alpha_base, color='gray', linestyle='--', linewidth=1.5,
           alpha=0.7, label=rf'$\alpha_{{\text{{base}}}} = {alpha_base}$')
ax.axhline(y=alpha_base * (1 + A), color='lightgray', linestyle=':', 
           linewidth=2, alpha=0.7)
ax.axhline(y=alpha_base * (1 - A), color='lightgray', linestyle=':', 
           linewidth=1.5, alpha=0.7)

ax.text(4*T + 0.3, alpha_base * (1 + A), 
...
ax.annotate('', xy=(second_peak_time, y_bracket), xytext=(first_peak_time, y_bracket),
            arrowprops=dict(arrowstyle='<->', color='black', lw=1.5))
ax.text((first_peak_time + second_peak_time)/2, y_bracket + 0.3, 'T', 
...
# legend
ax.legend(bbox_to_anchor=(1.25, 0.5), loc='center right', frameon=True, fancybox=True, 
          shadow=False, fontsize=13, framealpha=0.95,
