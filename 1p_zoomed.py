...
ax.plot(t, p_lacI, color='#DC143C', linewidth=1.8, label='LacI (forced)', alpha=0.9, zorder=3)
ax.plot(t, p_cI, color='#0066CC', linewidth=1.0, label='λcI', alpha=0.9, zorder=3)

# centerline
if global_center != 0:
    ax.axhline(y=global_center, color='gray', linestyle=':', linewidth=1, alpha=0.6, zorder=2)
    ax.text(window_start + 0.2, global_center - 0.2, 'Oscillation Center (LacI)', color='gray', fontsize=11)

if t_p1 is not None:
    amp_x_range = t_window[peak1_idx-30:peak1_idx+30]

    amp_x = t_p1
    ax.annotate('', xy=(amp_x, y_p1), xytext=(amp_x, global_center),
                arrowprops=dict(arrowstyle='<->', color=c_annot, lw=1.5))
    
    ax.text(amp_x + 0.3, (y_p1 + global_center)/2, 
            f'Amplitude\n$A = {global_mean_A:.2f}$', 
    ...
    ax.annotate('', xy=(t_p1, y_dims), xytext=(t_p2, y_dims),
                arrowprops=dict(arrowstyle='<->', color=c_annot, lw=1.5))

    ax.text((t_p1 + t_p2)/2, y_dims + 0.05, 
