...
solution = odeint(repressilator_odes_forced, y0, t, args=(alpha_base, A, T, alpha_0, n, beta))
...
if len(peaks_lacI) >= 2 and len(peaks_tetR) >= 1 and len(peaks_cI) >= 1:
    ...
    t_peak_tetR = tet_candidates[0] if len(tet_candidates) > 0 else t_window[peaks_tetR[0]]
    
    p_peak_lacI_1 = p_lacI_window[peaks_lacI[0]]
    p_peak_lacI_2 = p_lacI_window[peaks_lacI[1]]
    p_peak_tetR = p_tetR_window[peaks_tetR[np.where(t_window[peaks_tetR] == t_peak_tetR)[0][0]]]
...
ax.plot(t, p_lacI, color=c_lacI, linewidth=1.8, label='LacI (forced)', alpha=0.9, zorder=3)
...
def draw_drop_line(t_val, y_start, y_end, color):
    ax.plot([t_val, t_val], [y_start, y_end], linestyle=':', color=color, 
...
if t_peak_lacI_1 is not None:
    ...
    ax.annotate('', xy=(t_peak_cI, arrow_y_low), xytext=(t_peak_lacI_1, arrow_y_low), arrowprops=arrow_props)
    ax.text((t_peak_lacI_1 + t_peak_cI)/2, arrow_y_low + 0.05, f'$\\tau_1 = {tau1_disp:.2f}$', 
            ha='center', va='bottom', fontsize=11, color=c_annot)

    ax.annotate('', xy=(t_peak_tetR, arrow_y_mid), xytext=(t_peak_lacI_1, arrow_y_mid), arrowprops=arrow_props)
    ax.text((t_peak_lacI_1 + t_peak_tetR)/2, arrow_y_mid + 0.05, f'$\\tau_2 = {tau2_disp:.2f}$', 
            ha='center', va='bottom', fontsize=11, color=c_annot)

    ax.annotate('', xy=(t_peak_lacI_2, arrow_y_high), xytext=(t_peak_lacI_1, arrow_y_high), arrowprops=arrow_props)
    ax.text((t_peak_lacI_1 + t_peak_lacI_2)/2, arrow_y_high + 0.05, 
            f'Period $T = {T_disp:.2f}$',
            ha='center', va='bottom', fontsize=12, color=c_annot)
...
if phase_1_disp is not None:
    ...
    ax.text(1.03, 0.6, textstr, transform=ax.transAxes,
...
ax.legend(bbox_to_anchor=(1.012, 1), loc='upper left', frameon=True, fancybox=True, 
          fontsize=13, framealpha=0.95, edgecolor='gray', borderpad=0.8)
