...
print(f"Starting sweep over {len(forcing_periods)} periods...")
...
# plot 1: Period
ax1.axhline(y=natural_period, color='gray', linestyle=':', linewidth=1.5, label='Natural Period $T_0$', alpha=0.6)

ax1.plot(forcing_periods, response_periods, 'o-', color='#0066CC', linewidth=1.5, markersize=4, label='Period $T_{LacI}$')

ax1.set_ylabel('Period $T_{LacI} [mRNA lifetimes]$', fontsize=16)
ax1.legend(loc='upper left', frameon=True, fancybox=True, fontsize=13, framealpha=0.95, edgecolor='gray', borderpad=0.8)
ax1.grid(True, alpha=0.25, linestyle='-', linewidth=0.5)

# plot 2: Amplitude
ax2.axvline(x=natural_period, color='gray', linestyle=':', linewidth=1.5, label='Natural Period $T_0$', alpha=0.6)
ax2.plot(forcing_periods, response_amplitudes, 'o-', color='#DC143C', linewidth=1.5, markersize=4, label='Amplitude $A_{LacI}$')

ax2.set_ylabel('Amplitude $A_{LacI}$[$K_M$]', fontsize=16)
ax2.set_xlabel('Forcing Period $T$ [mRNA lifetimes]', fontsize=16)

ax2.legend(loc='upper right', frameon=True, fancybox=True, fontsize=13, framealpha=0.95, edgecolor='gray', borderpad=0.8)
