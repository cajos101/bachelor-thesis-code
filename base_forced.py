...
if steady_start_time < t[-1]:
    ax.axvspan(0, steady_start_time, alpha=0.12, color='orange', label='Transient window', zorder=0)
    ax.axvspan(steady_start_time, t[-1], alpha=0.12, color='lightblue', label='Steady window', zorder=0)
else:
    ax.axvspan(0, t[-1], alpha=0.12, color='orange', label='Transient (Not Settled)', zorder=0)

# plot
ax.plot(t, p_tetR, color='#FF8C00', linewidth=1.8, label='TetR (forced)', alpha=0.9, zorder=3)
ax.plot(t, p_lacI, color='#DC143C', linewidth=1.8, label='LacI (forced)', alpha=0.9, zorder=3)
ax.plot(t, p_cI, color='#0066CC', linewidth=1.8, label='λcI (forced)', alpha=0.9, zorder=3)
