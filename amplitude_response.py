...
print(f"Starting sweep over {len(A_values)} amplitude values...")
...
for i, A in enumerate(A_values):
    ...
    if i % 10 == 0:
        print(f"Processed A={A:.2f}")

fig, ax = plt.subplots(figsize=(14, 7))

ax.plot(A_values, amps_TetR, color='#FF8C00', linewidth=1.8, label='TetR', zorder=3)
ax.plot(A_values, amps_LacI, color='#DC143C', linewidth=1.8, label='LacI (Forced)', zorder=3)
...
ax.text(0.535, ax.get_ylim()[1]*0.69, r'$A=0.5$', ha='center', va='top', fontsize=12, color='gray')
